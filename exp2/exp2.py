...
def draw_sin_and_cos(stu_id: str, stu_name: str, save_path: str):
    ...
    plt.ylabel("函数值（Y）")
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda val, pos: '{:g}'.format(val)))  # 修改y轴刻度标签格式

    x_ticks = np.arange(-2 * np.pi, 2 * np.pi + 1, np.pi)  # 设置x轴刻度值；+1是为了包含2π
...
def draw_heart_saddle(stu_id: str, stu_name: str, save_path: str):
    """
    绘制心形线和马鞍面
    ...
    fig.suptitle(f"心形线和马鞍面 {stu_id} {stu_name}")
    ...
    ax1.set_rlabel_position(45)  # 设置r标签的位置
    ...
    fig.suptitle(f"不同角度观察马鞍面 {stu_id} {stu_name}")
    ...
    plt.tight_layout()
    plt.savefig(save_path.replace('.png', '_angles.png'), dpi=300)
    plt.show()
